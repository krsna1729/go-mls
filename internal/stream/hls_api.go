...
func ApiWatchInputHLS(hlsMgr *HLSManager, streamMgr *StreamManager) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(parts) != 2 {
			streamMgr.Logger.Error("Invalid HLS request path", "path", r.URL.Path)
			http.NotFound(w, r)
			return
		}
		inputName, file := parts[0], parts[1]
		if inputName == "" || file == "" {
			streamMgr.Logger.Error("Missing inputName or file in HLS request", "path", r.URL.Path)
			http.NotFound(w, r)
			return
		}
...
func ApiStartHLSViewer(hlsMgr *HLSManager, streamMgr *StreamManager) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req struct {
		...
		if err := httputil.DecodeJSON(r, &req); err != nil {
			streamMgr.Logger.Error("HLS start viewer: failed to decode request", "err", err)
			httputil.WriteError(w, http.StatusBadRequest, "Invalid request")
			return
		}

		if req.InputName == "" {
			streamMgr.Logger.Error("HLS start viewer: missing input name")
		...
		if err != nil {
			streamMgr.Logger.Error("HLS start viewer: failed to add viewer", "inputName", req.InputName, "err", err)
			httputil.WriteError(w, http.StatusInternalServerError, "Failed to start HLS viewer")
			return
		}

		streamMgr.Logger.Info("HLS viewer started", "inputName", req.InputName, "viewerID", viewerID)
...
func ApiStopHLSViewer(hlsMgr *HLSManager, streamMgr *StreamManager) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req struct {
			InputName string `json:"input_name"`
		...
		if err := httputil.DecodeJSON(r, &req); err != nil {
			streamMgr.Logger.Error("HLS stop viewer: failed to decode request", "err", err)
			httputil.WriteError(w, http.StatusBadRequest, "Invalid request")
			return
		}

		if req.InputName == "" || req.ViewerID == "" {
			streamMgr.Logger.Error("HLS stop viewer: missing input name or viewer ID")
		...
		streamMgr.Logger.Info("HLS viewer stopped", "inputName", req.InputName, "viewerID", req.ViewerID)
